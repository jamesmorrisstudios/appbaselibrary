...
public abstract class BaseRecycleViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
    ...
    protected abstract void bindHeader(BaseRecycleHeaderItem headerItem);
    protected abstract void bindItem(BaseRecycleItem item);

    /**
     * Binds the given data to this view.
     * @param data Data to bind
     */
    public void bindItem(@NonNull final BaseRecycleContainer data) {
        if(isHeader) {
            bindHeader(data.getHeaderItem());
        } else {
            bindItem(data.getItem());
        }
    }
